...
#
#
# Taken from https://github.com/jjrob13/sklearn_cotraining/blob/master/sklearn_cotraining/classifiers.py
#
#
...
class CoTrainingClassifier(object):
    ...
    def predict(self, X1, X2):
        ...
        proba_supported = self.supports_proba(
            self.clf1_, X1[0]
        ) and self.supports_proba(self.clf2_, X2[0])
        ...
        for i, (y1_i, y2_i) in enumerate(zip(y1, y2)):
            if y1_i == y2_i:
                ...
                sum_y_probs = [
                    prob1 + prob2
                    for (prob1, prob2) in zip(y1_probs, y2_probs)
                ]
    ...
    def decision_function(self, X1, X2):
        ...
        y1_proba = self.clf1_.decision_function(X1)
        y2_proba = self.clf2_.decision_function(X2)

        #_epsilon = 0.0001
        #assert all(abs(sum(y_dist) - 1) <= _epsilon for y_dist in y_proba)
        return y1_proba, y2_proba
